 ...
 */
export function renderer({ promises, tx, folderName }) {
  /**
...
async function createFolder(promises, folderName) {
  ...
  await promises.mkdir(folderName || "renderer");
}
...
async function unzipAndCleanup({ data, promises, folderName }) {
  ...
  await extract("downloaded.zip", {
    dir: path.resolve(folderName || "renderer"),
  });
